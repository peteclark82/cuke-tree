...
module.exports = function(options, callback) {
		...
		if (data.skeleton) {
			...
			runTests({
				...
				stdout : function(stdout) {
					...
					while (match != null) {
						scenarioCount = parseInt(match[1]);
				...
				finishedCallback : function(code) {
					console.log(color("\n------------------------------------------------------------", "black+white_bg"));
					if (code > 0) { 
						console.log(color("Cucumber Skeleton Process Exited with code : "+ code, "red+bold"));
						runningSocket.emit("testsError", { error : "Exited with code : "+ code });
					} else {
						console.log(color("Cucumber Skeleton Run Process Exited with code : "+ code, "green+bold"));
						runTests({
							feature : data && data.feature ? data.feature : config.featuresPath,
							output : jsonFilename,
							skeleton : false,
	...
	function runTests(options) {
		console.log(color("\n------------------------------------------------------------", "black+white_bg"));
		console.log(color("Running cucumber"+ (options.skeleton ? " skeleton " : " ") +"feature path :-\n  "+ options.feature, "yellow+bold"));
		...
		cukeTree.runTests({
			...
			stdout : options.stdout ? options.stdout : function(stdout) {
				if (options.scenarioCount) {
					...
					var match = re.exec(stdout);
					while (match != null) {
						var testIndex = parseInt(match[1]);
						runningSocket.emit("testsProgressed", { percent : Math.round((100 / options.scenarioCount) * testIndex) });
						match = re.exec(stdout);
					}
				}
			}
