  ...
  cukeTree.createContext(config, function (err, ctContext) {
      ...
      .on("connection", function (socket) {
        runningSocket = socket
          ...
          .on("getReport", onGetReport)
					.on("getReportJson", onGetReportJson)
          .on("edit", edit);
    ...
    function onGetReport(data) {
      ...
      cukeTree.internal.pipelines.profiles.report.default(ce.cloneextend(ctContext, {
		...
		function onGetReportJson(data) {
      console.log(color("\n------------------------------------------------------------", "black+white_bg"));
      console.log(color("Getting report JSON", "yellow+bold"));
      console.log(color("\n------------------------------------------------------------", "black+white_bg"));
			cukeTree.internal.pipelines.profiles.report.default(ce.cloneextend(ctContext, {
        input: path.join(resultsPath, data.filename)
      }), function (err, result) {
        console.log(color("\n------------------------------------------------------------", "black+white_bg"));
        if (err) {
          console.log(color("Error getting report json :-", "red+bold"));
          console.log(err);
          runningSocket.emit("reportError", { error: err });
        } else {
          console.log(color("Report json successfully retrieved", "green+bold"));
          runningSocket.emit("reportReceivedJson", { json : result.renderData.structure, html : result.data });
          getTestHistory();
          updateGlossary();
        }
        console.log(color("\n------------------------------------------------------------", "black+white_bg"));
