...
$(function() {
	...
	$.fn.cukeTreeIde = function() {
		...
		function onReportReceivedJson(options) {
      var reportHtml = options.html;
      var reportJson = options.json;
			$this.trigger("cukeTreeIde.reportReceived", reportHtml);
			
      cukeTreeIde.events.uiEnabled();
			updateStatus("Displaying report for '"+ $.stateManager.get("filename") +"'...");
			$elements.display.summary.html(reportHtml.summary);
			$elements.tabs.stepDefinitions.html(reportHtml.steps).trigger("stepDefinitions.update");
			$elements.tabs.testDomain.html(reportHtml.domain).trigger("testDomain.update");
			
			structureJson = reportJson;
			$cukeTree.cukeTree().updateReport($elements.reportTrees.byFolder);
		}
