(function($) {	
	...
	function initialiseReports($reports) {
        ...
        function getTreeNode(type, reportEntry) {
          ...
          var isOpened = reportEntry.result && reportEntry.result.status == "failed" && type !== "scenario";
                  
          var node = {
            attr : { id : reportEntry.nodeId, "data-node-path" : reportEntry.nodePath, "data-type" : type, class : cssClasses },
            state : isOpened ? "open" : "closed",
            data : {
              attr : { href : "javascript:;" },
              title : reportEntry.name,
              icon : type
            }
          };
