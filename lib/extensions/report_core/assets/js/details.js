...
$(function () {
	$.fn.reportDetail = function() {
      ...
      /*
      var $embeddings = $node.find("div.embeddings");
      
      $embeddings.each(function (index) {
        var $embedding = $(this);
        var $embeddingLink = $embedding.find("a");
        var $embeddingImage = $embedding.find("img");        
        $embeddingLink.attr("href", $embeddingImage.attr("src"));
      });*/
        ...
        $cloned.find("a").each(function() { $(this).removeAttr("href"); });
        ...
        $embedInfo = $item.find("li[data-embedding-mime-type]");
        if ($embedInfo.length > 0) {
          var data = $embedInfo.data("embedding-data");
          var mimeType = $embedInfo.data("embedding-mime-type");
          var dataUrl = "data:"+ mimeType +";base64,"+ data;
          var $embed = $('<div class="embeddings"><a target="_blank" href="'+ dataUrl +'"><img src="'+ dataUrl +'" /></a></div>');
          $info.append($embed);
        }
			});	
