...
function generateReport(config, callback) {
	...
	function buildReport(code) {
		cukeTree.internal.pipelines.profiles.report.default({
			...
			hooks : [ loadedExtensions.attachHooks ],
			templates : loadedExtensions.templates,
			assets : loadedExtensions.assets
		}, function(err, result) {
			if (err) {callback(err);} else {
				console.log("Saving CukeTree report...");
				saveReport(config.output, result.data.page, result.glossaryJson, function(err) {
					if (err) {callback(err);} else {
	...
	function saveReport(outputPath, report, glossaryJson, callback) {
		var outputFile = path.join(outputPath, "report.htm");
		console.log("Building CukeTree report structure...");
		
		var ctExecutingContext = { input : config.input, output : outputPath, assets : loadedExtensions.assets };
